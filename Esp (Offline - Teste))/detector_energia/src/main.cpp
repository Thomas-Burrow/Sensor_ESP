...
bool readSensor(SensorData &out) {
    ...
    // --- Máquina de estados da rede elétrica ---
    switch (redeState) {
        ...
        case RedeState::QUEDA:
            ...
            // Durante a queda: sensor não consegue ler → retorna false
            return false;
    ...
    // --- Gerar valores conforme o estado ---
    float baseVolt, baseAmp, baseFreq;

    switch (redeState) {
        case RedeState::NORMAL:
            // Rede estável com pequena variação aleatória
            baseVolt = 219.0f + (random(-20, 20) * 0.1f);  // 217..221 V
            baseAmp  =   1.2f + (random(-10, 10) * 0.01f); // 1.1..1.3 A
            baseFreq =  60.0f + (random(-2, 2)   * 0.1f);  // 59.8..60.2 Hz
            break;

        case RedeState::INSTAVEL:
            // Oscilações maiores
            baseVolt = 200.0f + (random(-50, 50) * 0.5f);  // 175..225 V
            baseAmp  =   1.5f + (random(-30, 30) * 0.05f); // variável
            baseFreq =  59.5f + (random(-10, 10) * 0.1f);  // 58.5..60.5 Hz
            break;

        case RedeState::RETORNO:
            // Tensão subindo gradualmente
            baseVolt = 150.0f + (ciclosNoEstado * 14.0f);  // sobe ciclo a ciclo
    ...
    out.timestamp = 0;  // preenchido depois
...
void setup() {
    ...
    // Aguarda NTP sincronizar antes de continuar
